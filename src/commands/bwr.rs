...
pub async fn bwr(ctx: Context<'_>, map: GlobalMap, course: u8, mode: Mode) -> Result<()> {
	let map = MapIdentifier::Name(map.name);

	let tp_wr = global_api::get_wr(map.clone(), mode, true, course, &ctx.gokz_client)
		.await
		.ok();

	let pro_wr = global_api::get_wr(map.clone(), mode, false, course, &ctx.gokz_client)
		.await
		.ok();

	if tp_wr.is_none() && pro_wr.is_none() {
		return ctx.reply("No BWRs found.").await;
	}
	...
	ctx.reply(format!(
		"[WR on {map} B{course} ({mode})] TP: {tp_time} | PRO: {pro_time}",
		mode = mode.short()
	))
	.await
}
